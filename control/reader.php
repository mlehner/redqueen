...
//
// RFID Tag Reader Daemon, for Buffalo Hacker Space
//
// Version: 1.0
// Copyright: synace
//            http://synace.com
...
include('encrypt-key.php');
sleep(15);
exec('midori -c ' . dirname(__FILE__).'/midori https://10.27.0.1/?door=MAIN');

// Parallax USB RFID Reader 32395
// 2400 8-N-1
$serial = new phpSerial();
if ($serial->deviceSet("/dev/ttyUSB0")) {
  $serial->confBaudRate(2400); //2400
  $serial->confCharacterLength(8); //8
  $serial->confParity("none"); //N
  $serial->confStopBits(1); //1
  ...
  while(1) {
    ...
    if ($data != null) {
      $tag = trim(trim(trim($data, "\x02..\x03"), "\x00..\x1F"));
      if (strlen($tag)) {
        //var_dump($tag);
        $serial->deviceClose();
	...
	if (preg_match('#^([0-9A-Z]{10})#', $tag, $matches)) {
          ...
          exec('xset -display :0 s reset > /dev/null &');
          exec('xset -display :0 dpms force on > /dev/null &');
        }
        usleep(2000000);
        $serial->deviceOpen();
      }
